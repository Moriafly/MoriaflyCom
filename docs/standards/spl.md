...
- SPL 在对适配各种兼容性问题的解决方案上将它们视为一种“语法糖”，并进行**标准化，对编辑更加友好方便**，同时也是本标准的意义。
...
其中 `[05:20.22]` 便是一个时间戳，表示 5 分 20 秒 220 毫秒，分和秒之间由 `:`（半角冒号）分隔，秒和毫秒间由 `.`（半角句号）分隔，这和日常书写习惯一致。
...
时间戳后不接任何文本内容的行是纯粹的结束标记：它不会产生一句新歌词，也不参与[翻译识别](#翻译识别)，即无论它与哪句歌词时间戳相同，都不会被视为主歌词或翻译文本。

实际歌曲文件中，结束标记行常与下一句歌词使用相同的时间戳，如：
...
其中第二行 `[05:21.22]` 空行表示 `你好椒盐音乐` 在 5 分 21 秒 220 毫秒结束，与 `天天开心` 从 5 分 21 秒 220 毫秒开始并不冲突。
...
逐字歌词是可以针对一行歌词内的文本进行更精确的时间控制，不局限于对每个字符都精确控制，也可以是词组或者其他一部分，如：
